...
namespace Envelope.EnterpriseServiceBus.PostgreSql.Writers.Internal;
...
internal class JobMessageWriter : IJobMessageWriter, IJobMessagePublisher
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		traceInfo = TraceInfo.Create(traceInfo);
		message.Archive(traceInfo, properties, detail, isDetailJson);
		...
		if (removeFromActiveMessages)
		{
			...
			try
			{
				...
				{
					await using var martenSession = _store.OpenSession();
					martenSession.Delete(activeJobMessage);
					await martenSession.SaveChangesAsync(cancellationToken).ConfigureAwait(false);
				}
				else
				{
